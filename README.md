...
The tester portal is `https://preview.recordcove.com`. The exact archive is linked by the manifest-pinned public GitHub prerelease after its revision, byte size, SHA-256 checksum, bundle identity, and ad-hoc signature are verified. Current large bundled-model archives use the immutable `downloads.preview.recordcove.com` object path; earlier accepted previews may remain GitHub-hosted. Testers do not need a GitHub account.

The Downloads page reads the public GitHub prerelease inventory and displays only releases at or below the manifest-pinned accepted preview. The newest metadata-only release must name the same immutable download URL that the manifest binds to the exact byte size and SHA-256 digest. Historical GitHub assets retain their provider digest checks. A newer staged prerelease is therefore not promoted before its manifest is accepted. The page remembers the last selected version only in that browser's local storage; no download history is uploaded.
...
The next bundled-model preview includes verified Whisper Small English and IBM Granite 4.1 3B inside the application. Whisper is the default transcription model and Granite is the default Understanding model for titles, summaries, and enhancement. Testers must not need a second Granite download or be silently moved to Apple on-device. The bundled-model archive exceeds GitHub Release's single-asset limit. Its exact accepted byte count and SHA-256 checksum must come only from the immutable release manifest generated from the final verified package, never from a hand-maintained README value. The protected publisher uses multipart staging in `recordcove-preview-downloads`, verifies the staged bytes, conditionally promotes an unused immutable key, verifies the final object, and then hashes the complete public response from `downloads.preview.recordcove.com` before staging the manifest. Provider credentials never enter the M3 build runner.
...
5. Upload the exact archive only through the protected immutable R2 publisher and keep the GitHub prerelease metadata-only for the bundled-model lane.
